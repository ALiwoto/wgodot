...
bool ExportAnalysis::is_global_class(const StringName &p_name) {
	return active != nullptr ? active->global_classes.has(p_name) : ScriptServer::is_global_class(p_name);
}

String ExportAnalysis::get_global_class_path(const StringName &p_name) {
	if (active == nullptr) {
		return ScriptServer::get_global_class_path(p_name);
	}
	const String *path = active->global_classes.getptr(p_name);
...
StringName ExportAnalysis::get_global_class_native_base(const StringName &p_name) {
	...
	if (active->get_source(path) != nullptr) {
		Error error = OK;
		Ref<GDScriptParserRef> ref = active->get_parser(path, GDScriptParserRef::INHERITANCE_SOLVED, error);
		return error == OK ? ref->get_parser()->get_tree()->get_datatype().native_type : StringName();
	}
	const StringName *base = active->external_native_bases.getptr(path);
	return base != nullptr ? *base : StringName();
}

Ref<Script> ExportAnalysis::load_script(const String &p_path, const String &p_type) {
...
bool ExportAnalysis::resource_exists(const String &p_path) {
	return (active != nullptr && active->get_source(p_path) != nullptr) || ResourceLoader::exists(p_path);
}

String ExportAnalysis::get_resource_type(const String &p_path) {
