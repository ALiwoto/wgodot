...
void ExportContext::index_script(const GDScriptParser::ClassNode *p_class, const String &p_script_path) {
	...
	if (total_usec >= slow_threshold_usec || name_probe.usec >= slow_threshold_usec || probe.get_or_create_member_rename_usec >= slow_threshold_usec) {
		WARN_PRINT(export_timing_prefix() + vformat("index_script %s total=%s ms classes=%d members=%d obfuscatable=%d reserve_script_global=%s reserve_member=%s member_get_name=%s make_keys=%s get_or_create_member=%s bind=%s name_calls=%d name_time=%s name_attempts=%d name_collisions=%d name_max_attempts=%d member_renames=%d reserved_member_names=%d", p_script_path, export_timing_format_msec(total_usec), probe.classes, probe.members, probe.obfuscatable_members, export_timing_format_msec(probe.reserve_script_global_usec), export_timing_format_msec(probe.reserve_member_name_usec), export_timing_format_msec(probe.member_get_name_usec), export_timing_format_msec(probe.make_member_keys_usec), export_timing_format_msec(probe.get_or_create_member_rename_usec), export_timing_format_msec(probe.bind_member_rename_usec), name_probe.calls, export_timing_format_msec(name_probe.usec), name_probe.attempts, name_probe.collisions, name_probe.max_attempts, member_renames.size(), reserved_member_names.size()));
	}
}
