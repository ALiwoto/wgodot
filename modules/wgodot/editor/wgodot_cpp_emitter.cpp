...
WGodotCppEmitter::Value WGodotCppEmitter::member(const Parser::ExpressionNode *p_base, const StringName &p_name, const Parser::ExpressionNode *p_origin) {
	...
	if (entry.type == Parser::ClassNode::Member::CONSTANT) {
		if (container_constant_owners.has(entry.constant)) {
			return container_constant(entry.constant);
		}
		// Lowering the value records its own dependencies; the declaring class
		// may contain only inlined constants and have no generated header.
...
String WGodotCppEmitter::leaf_expression(const Parser::ExpressionNode *p_expression) {
	...
	if (can_inline_constant(p_expression) && !p_expression->type_constraint.is_meta_type) {
