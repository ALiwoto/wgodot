...
String WGodotCppEmitter::native_interface_call(const Parser::CallNode *p_call, const Parser::ExpressionNode *p_base, const WGodotNativeInterfaces::Descriptor &p_interface) {
	...
	for (const PropertyInfo &parameter : method.arguments) {
		...
		String value;
		if (supplied) {
			value = engine_argument(p_call->arguments[index], parameter.type);
		} else {
		...
		const String argument = "argument_" + itos(index);
