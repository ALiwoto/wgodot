...
Use breakpoint CRUD, named and conditional logical breakpoints, one-shot breakpoints, hard debugger pausing, stepping, stack frames, scoped variables, and wait commands as described in [references/debugging.md](references/debugging.md). Prefer `debug wait` over polling when waiting for a breakpoint.
